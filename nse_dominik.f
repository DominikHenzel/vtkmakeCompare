      ...
      parameter(nx=4,ny=256,nz=256) ! Number of gridpoint
      parameter(dt=1.0D-3)    ! time step 
      parameter(Re=20.0D0)      ! Reynolds number
      parameter(Sc=1.0d0)   !Schmidt number
      parameter(xl=2.5d-1)
      parameter(yl=4.0d0)
      parameter(zl=4.0d0)       !domain size
!     parameter(nnn=100000000) !number of iteration
      ...
      parameter(eta=1.0d2/dt) !for VPM term
...
!--------------------------reading part----------------------------
      ...
      if(int_rg3d.eq.1)then
         ...
         call set_levelset(nx,ny,nz,dx,dx12,dy12,dz12,
...
c     &        nx,ny,nz,res,res_max,res_int,res_var)!KAMETANI
...
!----------------------------DATA OUTPUT---------------------------------
         ...
         IF(MOD(L,100) .EQ. 0)THEN
            WRITE(*,*) 'CALL OUTPUT'
            CALL OUTPUT
     &     (u,v,w,p,x,y,z,nx,ny,nz,T,cellvalue,
     &     u_ta,v_ta,w_ta,phi,divu)
...
c     write instantanous crossection data
     ...
     & fi')
      ...
      open(1009,file='./nseoutput2/cellvalue1.dat',form='unformatted')
      open(1010,file='./nseoutput2/cellvalue2.dat',form='unformatted')
      open(1011,file='./nseoutput2/cellvalue3.dat',form='unformatted')
      open(1012,file='./nseoutput2/cellvalue4.dat',form='unformatted')
      do k=1,nz
         do j=1,ny
            do i=1,nx
               ...
               write(1009)cellvalue(i,j,k,1)
               write(1010)cellvalue(i,j,k,2)
               write(1011)cellvalue(i,j,k,3)
               write(1012)cellvalue(i,j,k,4)
            enddo
         enddo
      enddo
